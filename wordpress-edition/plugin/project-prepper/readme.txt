...
* Quick actions on the dashboard: add an item, start a rental, inquiry or event, and — inside a collective — a project, a poll or an invitation. Each tile opens the form directly on the target page.
* Collectives are compact tiles that link to the collective, the profile is a slim card beside them, and the two share a row on wide screens.
* Note: this update does not change the database.

= 0.142.0 =
* The page title now sits in the top bar, next to the greeting and a live clock showing weekday, date and time in the site's time zone. Content pages keep their intro line but no longer repeat the title.
* Clicking your profile picture takes you to your dashboard in the solo workspace — one click to get back to your own things from any collective.
* "How the platform works" is a menu item of its own instead of a banner that opened on every dashboard visit.
* "New rental" moved up under the page header, above the summary tiles.
* Note: this update does not change the database.
